...
- **この泳者の泳ぎ方（検出の前提）**: クイックターン不可＝**オープンターン**で毎回足を底につけて立ってから壁を蹴る。
  休憩は**ベンチに腰掛け**（その前後に約10秒の立位/歩行）。狙いは 100m×5×2＝1000mを大休憩で半分割。ペース≒30s/25m。
- **gz オリエンテーション検出 (`analysis/turn_stand.py`)**: 立位/着座で体軸が垂直化し、重力 z が
  **泳ぎ -7.5 ↔ 立位 +9.5 m/s²** に振れる。これが最も明確な壁シグネチャ（グライドギャップ/フリップ回転は弱く不適）。
  ...
  turn(1.2-7s)=ターン壁 / rest(7-90s)=ベンチ休憩=レップ境界 / big-rest(>90s)=ハーフ分割。lengths = walls+1。
- **3タップは gz 領域で復元可**: jerk 領域では泳ぎに埋もれ不能（壁タッチ/ストロークと混在）だが、
  立位で打った 0.9s×3連は gz-立位の三連ブリップとして分離できた（901s/938s）。
- **結果**: block1 ≒21レングス(525m, 意図500m・rep2-4は教科書通り3ターン100m), block2 ≒33レングス(825m, 連続的で
  ベンチ着座が浅くレップ境界が曖昧)。合計 ≒54レングス1350m（体感1000mより多い）。
- **外部GTが無い問題**: プールにスマホ持込不可＋本人も覚えていられない＝**3タップが唯一のGT機構だったが水中タップは不能**。
  → 検出は内的整合性（等間隔性）+妥当ペースで検証するしかない。タップは「壁で静止し立位で」打てば gz で拾える可能性。
